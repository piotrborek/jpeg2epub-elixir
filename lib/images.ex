defmodule Images do
  defmodule ParseState do
    ...
    def update_current(%ParseState{} = parseState, char),
      do: %ParseState{parseState | current: [char | parseState.current]}

    @spec update_output(ParseState.t(), :int | :char, String.t()) :: ParseState.t()
    def update_output(%ParseState{} = parseState, type, char),
      do: %ParseState{
        parseState
        | previous: type,
          current: [char],
    ...
    def to_list(%ParseState{} = parseState),
      do: [to_int_or_string(parseState) | parseState.output]
  ...
  def sort_images(list) do
    list
    |> Enum.map(fn str -> [str, parse_string(str |> Path.rootname())] end)
  ...
  def parse_string(text) do
    ...
    |> Enum.reduce(%ParseState{previous: :none}, fn char, acc ->
      ...
      case acc.previous do
        ...
        _ ->
          raise "Images.parse_string/1: Should never happen."
      end
    end)
